...
NEW_BRANCH="${COMMIT_TYPE}/${PR_BRANCH_NAME_PREFIX}_${NEW_TEMPLATE_GIT_HASH}"
...
gh pr create \
  --title "${PR_TITLE}" \
  --body "This PR will merge the [${SOURCE_REPO_PATH}](https://github.com${SOURCE_REPO_PATH}/) changes" \
  -B "${UPSTREAM_BRANCH}" \
  -l "${PR_LABELS}"
echo "::endgroup::"
